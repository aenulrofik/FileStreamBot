...
async def cb_data(bot, update: CallbackQuery):
    ...
    uids = update.from_user.id
    uid = int(usr_cmd[0])
    if uid != uids:
        await update.answer("Ini bukan tugas anda !!", show_alert=True)

    if usr_cmd[1] == "home":
        await update.message.edit_text(
            text=LANG.START_TEXT.format(update.from_user.mention, FileStream.username),
            disable_web_page_preview=True,
            reply_markup=BUTTON.START_BUTTONS(uid)
        )
    elif usr_cmd[1] == "help":
        await update.message.edit_text(
            text=LANG.HELP_TEXT.format(Telegram.OWNER_ID),
            disable_web_page_preview=True,
            reply_markup=BUTTON.HELP_BUTTONS(uid)
        )
    elif usr_cmd[1] == "about":
        await update.message.edit_text(
            text=LANG.ABOUT_TEXT.format(FileStream.fname, __version__),
            disable_web_page_preview=True,
            reply_markup=BUTTON.ABOUT_BUTTONS(uid)
        )
    ...
    elif usr_cmd[1] == "close":
        await update.message.delete()
    elif usr_cmd[1] == "msgdelete":
        await update.message.edit_caption(
        caption= "**Cᴏɴғɪʀᴍ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴅᴇʟᴇᴛᴇ ᴛʜᴇ Fɪʟᴇ**\n\n",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("ʏᴇs", callback_data=f"{uid}_msgdelyes_{usr_cmd[2]}_{usr_cmd[3]}"), InlineKeyboardButton("ɴᴏ", callback_data=f"{uid}_myfile_{usr_cmd[2]}_{usr_cmd[3]}")]])
    )
    elif usr_cmd[1] == "msgdelyes":
        await delete_user_file(usr_cmd[2], int(usr_cmd[3]), update)
        return
    elif usr_cmd[1] == "msgdelpvt":
        await update.message.edit_caption(
        caption= "**Cᴏɴғɪʀᴍ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴅᴇʟᴇᴛᴇ ᴛʜᴇ Fɪʟᴇ**\n\n",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("ʏᴇs", callback_data=f"{uid}_msgdelpvtyes_{usr_cmd[2]}"), InlineKeyboardButton("ɴᴏ", callback_data=f"{uid}_mainstream_{usr_cmd[2]}")]])
    )
    elif usr_cmd[1] == "msgdelpvtyes":
        await delete_user_filex(usr_cmd[2], update)
        return

    elif usr_cmd[1] == "mainstream":
        _id = usr_cmd[2]
        reply_markup, stream_text = await gen_link(_id=_id, uid=uid)
        await update.message.edit_text(
            text=stream_text,
    ...
    elif usr_cmd[1] == "userfiles":
        file_list, total_files = await gen_file_list_button(int(usr_cmd[2]), update.from_user.id)
        await update.message.edit_caption(
            caption="Total files: {}".format(total_files),
            reply_markup=InlineKeyboardMarkup(file_list)
            )
    elif usr_cmd[1] == "myfile":
        await gen_file_menu(usr_cmd[2], usr_cmd[3], update)
        return
    elif usr_cmd[1] == "sendfile":
        myfile = await db.get_file(usr_cmd[2])
...
async def gen_file_list_button(file_list_no: int, user_id: int):
    ...
    async for x in user_files:
        file_list.append([InlineKeyboardButton(x["file_name"], callback_data=f"{uid}_myfile_{x['_id']}_{file_list_no}")])
    if total_files > 10:
        file_list.append(
                [InlineKeyboardButton("◄", callback_data="{}".format(f"{uid}_userfiles_"+str(file_list_no-1) if file_list_no > 1 else f'{uid}_N/A')),
                 InlineKeyboardButton(f"{file_list_no}/{math.ceil(total_files/10)}", callback_data=f"{uid}_N/A"),
                 InlineKeyboardButton("►", callback_data="{}".format(f"{uid}_userfiles_"+str(file_list_no+1) if total_files > file_list_no*10 else f'{uid}_N/A'))]
        )
    if not file_list:
        file_list.append(
                [InlineKeyboardButton("ᴇᴍᴘᴛʏ", callback_data=f"{uid}_N/A")])
    file_list.append([InlineKeyboardButton("ᴄʟᴏsᴇ", callback_data=f"{uid}_close")])
